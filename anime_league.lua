-- ============================================================================
--  Anime League auto-load
-- ============================================================================
loadstring(game:HttpGet("https://raw.githubusercontent.com/iccyzsn/anime_league/refs/heads/main/anime_league.lua"))()

-- ============================================================================
--  ASSET ID TRACKER  •  v2.2  (hidden by default + precise click detection)
-- ============================================================================
...
local CONFIG = {
	...
	StartHidden         = true,          -- UI hidden until toggle pressed
...
-- little pulse ring so you can find it
...
create("TextLabel", {
	...
	Text = "Live Workspace monitor  •  v2.2",
...
local function addEntry(data)
	...
	--  Use InputBegan with target check so the copy button
	--  does NOT trigger row selection and vice versa.
	-- ==========================================================
	row.InputBegan:Connect(function(input)
		...
		-- Skip if the click was on the copy ID button
		if t == idBtn or (t and t:IsDescendantOf(idBtn)) then return end
		-- Only select if the click landed inside THIS row
		if t == row or (t and t:IsDescendantOf(row)) then
			selectEntry(record)
		end
	end)
...
-- click the minimize button → hide window back to floating bubble
...
-- Hide window when clicking on the toggle button bubble
-- (also pulse animation loop)
task.spawn(function()
...
print("╔══════════════════════════════════════════╗")
print("║   ASSET ID TRACKER v2.2  —  LOADED       ║")
